...
export function ImpulseProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
        ...
        setExcitementHistory((hist) => {
          ...
          if (level > prevLevel && level > InterventionLevel.L0_NORMAL) {
            ...
            triggerInfo = {
              ...
              reason:
                level >= 4
                  ? 'Rapid browsing + repeated visits'
                  : level >= 3
                  ? 'High intensity interaction'
                  : 'Extended viewing',
              timestampLabel: `${now.getHours().toString().padStart(2, '0')}:${now
                .getMinutes()
                .toString()
                .padStart(2, '0')}`,
            };
    ...
    return () => {
      // Use globalThis to avoid lib typing issues in React Native
      globalThis.clearInterval(id);
    };
  }, []);

  const setCurrentProduct = (p: Product | null) => {
